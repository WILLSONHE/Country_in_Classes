from process_updates import *


def main():
    # Main simply makes use of the function process_updates to process the files.
    # ... main provides two files: a "data file" with the geographical information and an "update file" with
    #      the list of updates
    # ... the function process_updates ensures that each exists, and, IF NOT, it will prompt for new file names or
    ...
    print()
    country_fname = input("Enter name of file with country data: ")
    update_fname = input("Enter name of file with country updates: ")
    result = process_updates(country_fname,update_fname)
    print()
    print(40*"*")
    if result:
